...
function Terminal() {
  const frame = useCurrentFrame();
  const command = "vault-env to .env --names OPENAI_API_KEY,GITHUB_TOKEN";
  const typed = command.slice(0, Math.floor(Math.max(0, frame - 136) * 1.45));
  const resultOpacity = interpolate(frame, [178, 190], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: ease });
  return <div style={{ width: 700, height: 310, border: "1px solid #293847", borderRadius: 18, background: "#0c131b", boxShadow: "0 22px 50px #0008", overflow: "hidden" }}><div style={{ height: 46, background: "#131f2b", display: "flex", alignItems: "center", gap: 9, padding: "0 17px" }}><i style={{ width: 10, height: 10, borderRadius: "50%", background: "#ff6b6b" }} /><i style={{ width: 10, height: 10, borderRadius: "50%", background: "#ffd166" }} /><i style={{ width: 10, height: 10, borderRadius: "50%", background: green }} /><span style={{ color: muted, marginLeft: 10, fontSize: 17 }}>project-a · terminal</span></div><div style={{ padding: 28, fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: 20, lineHeight: 1.7 }}><div style={{ whiteSpace: "nowrap" }}><span style={{ color: green }}>$</span> {typed}<span style={{ color: green }}>▌</span></div><div style={{ opacity: resultOpacity }}><div style={{ color: green, marginTop: 18 }}>✓ 2 secretos → .env</div><div style={{ color: muted, fontSize: 17 }}>claves: GITHUB_TOKEN, OPENAI_API_KEY</div><div style={{ color: cyan, marginTop: 16 }}>sin valores en stdout · permisos 600</div></div></div></div>;
}
...
function App() {
  ...
  return <AbsoluteFill style={{ background: "#071017", color: "#f4f7f8", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif", overflow: "hidden" }}>
    ...
    <Fade from={125} to={145} out={220} style={{ position: "absolute", left: 290, top: 190 }}><Terminal /></Fade>
    <Fade from={215} to={233} style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", textAlign: "center" }}><div><div style={{ transform: `scale(${pulse})`, fontSize: 95, color: green }}>✓</div><div style={{ fontSize: 56, fontWeight: 780, marginTop: 10 }}>Sin volver a copiar secretos.</div><div style={{ fontSize: 29, color: muted, marginTop: 15 }}>El agente arma el comando. El proyecto recibe el entorno.</div><div style={{ display: "flex", justifyContent: "center", gap: 12, marginTop: 30 }}><Badge>CONSOLA CENTRAL</Badge><Badge color={cyan}>.ENV / PROCESO</Badge><Badge>AGENT-SAFE</Badge></div></div></Fade>
